...
export default function ApplyCXO() {
  ...
  function validateStep1() {
    ...
    if (!form.teamSize) errs.teamSize = "Please select your organisation size";
    return errs;
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/create-checkout", {
        ...
        body: JSON.stringify({ ...form, source: "cxo-apply-form" }),
      });
  ...
  return (
    <div className="font-quicksand text-brand-text bg-brand-surface leading-relaxed min-h-screen">
      <PageMeta
        title="Apply for a Seat — AI Confidence Day for CXOs & Board Members"
        description="Secure your place at AI Confidence Day for CXOs & Board Members. 3rd July 2026, London."
        url="https://aiday.solvedtogether.co.uk/apply/cxo"
      />
            ...
            <form onSubmit={handleNextStep} noValidate className="space-y-5">
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. I use ChatGPT for emails and summarising reports, but I find it hard to know what else I should be doing with it at a leadership level..."
                  rows={6}
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
              ...
              <div className="border border-brand-border bg-white p-6 space-y-3">
                ...
                {[
                  ["Name", form.fullName],
                  ["Email", form.email],
                  form.businessName && ["Organisation", form.businessName],
                  ["Country", form.country],
                  form.phone && ["Phone", form.phone],
                  ["Organisation size", form.teamSize],
                  ["Role", form.jobRole],
                ]
                  .filter(Boolean)
                  .map(([label, value]) => (
                    <div key={label as string} className="flex justify-between text-sm gap-4">
                      <span className="text-brand-text/50 uppercase tracking-[0.5px] flex-shrink-0">{label}</span>
                ...
                <div className="flex gap-4 mt-2">
                  <button type="button" onClick={() => { setStep(1); setErrors({}); window.scrollTo({ top: 0 }); }} className="text-xs text-brand-primary underline hover:opacity-75">
                    Edit contact details
                  </button>
                  <button type="button" onClick={() => { setStep(2); setErrors({}); window.scrollTo({ top: 0 }); }} className="text-xs text-brand-primary underline hover:opacity-75">
                    Edit answer
                  </button>
                </div>
              </div>
              ...
              <div>
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm text-brand-text/80 leading-relaxed">
                    ...
                    <a href="/#apply" className="text-brand-primary underline underline-offset-2 hover:opacity-75">
